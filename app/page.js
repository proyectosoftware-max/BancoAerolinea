...
export default function Component() {
  ...
  const datosUsuario = useSearchParams();
  const nombre = datosUsuario.get('nombre');
  const apellido = datosUsuario.get('apellido');
  ...
  const handleSubmit = (event) => {
    ...
    router.push(`/clave?usuario=${encodeURIComponent(username)}&nombre=${encodeURIComponent(nombre)}&apellido=${encodeURIComponent(apellido)}`);
  };
